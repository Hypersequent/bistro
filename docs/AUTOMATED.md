...
`Automation = Automated` custom field, and the **§10 implementation spec** that makes the Playwright
...
(cart math, quantity rules, checkout flow, persistence, routing) rather than visual polish.
...
- **Automated as** — each case names its Playwright spec file and the exact `test(...)` title. The
  title embeds the `BD-###` marker so qas-cli can match the result to the case (see §10).
- **Precondition / steps** — reference the shared entities from `MANUAL.md`; §10 explains how each is
...
> this deliverable — §10 specifies them precisely (files, titles, config, upload) for implementation.
